...
ren.energy.bio        <- read_excel(paste(filepath.iams, "se_liquids_biomass.xlsx",sep=""))
...
ren.energy.bio        <- head(ren.energy.bio,-2)[,!names(ren.energy.bio) %in% c("Notes","Region","Variable","Unit")]  
colnames(ren.energy.hydro) <- colnames(ren.energy.solar) <- colnames(ren.energy.wind) <- colnames(ren.energy.bio) <- c(c("Model","Scenario"),as.character(c(2005,seq(2010,2100,by=10))))
ren.energy <-   rbind(ren.energy.solar,ren.energy.wind,ren.energy.hydro, ren.energy.bio)
