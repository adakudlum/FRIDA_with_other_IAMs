...
calib.df         <- read.csv2(paste(filepath.frida.calib,"/","Calibration Data.csv",sep=""), sep=",")[,-c(46:52)]
...
forest.land.calibration            <- filter(calib.df, if_any(everything(),~str_detect(.x,"Forest land")))
population.calibration             <- filter(calib.df, if_any(everything(),~str_detect(.x,"Population")))
ren.energy.calibration             <- filter(calib.df, if_any(everything(),~str_detect(.x,"Renewable Energy Output")))
fossil.energy.calibration          <- filter(calib.df, if_any(everything(),~str_detect(.x,"fossil energy.Secondary Fossil Energy Output")))
total.energy.calibration           <- filter(calib.df, if_any(everything(),~str_detect(.x,"Total Energy Output")))
animal.products.demand.calibration <- filter(calib.df, if_any(everything(),~str_detect(.x,"Food Demand.Animal Products Demand")))
slr.calibration                    <- filter(calib.df, if_any(everything(),~str_detect(.x,"Total global sea level anomaly")))

# format data frame
gdp.per.person.calibration         <- data.frame(as.factor(seq(1980,2150)),abind(as.numeric(t(gdp.per.person.calibration[,-1]))/deflator.value.2005,rep(NA,len.frida.data-44)))
sta.calibration                    <- data.frame(as.factor(seq(1980,2150)),abind(as.numeric(t(sta.calibration[,-1])),rep(NA,len.frida.data-44)))
forest.land.calibration            <- data.frame(as.factor(seq(1980,2150)),abind(as.numeric(t(forest.land.calibration[,-1]))*1e-3,rep(NA,len.frida.data-44)))
population.calibration             <- data.frame(as.factor(seq(1980,2150)),abind(as.numeric(t(population.calibration[,-1]))*1e-3,rep(NA,len.frida.data-44)))
animal.products.demand.calibration <- data.frame(as.factor(seq(1980,2150)),abind(as.numeric(t(animal.products.demand.calibration[,-1]))*1e-3,rep(NA,len.frida.data-44)))
ren.energy.calibration             <- data.frame(as.factor(seq(1980,2150)),abind(as.numeric(t(ren.energy.calibration[,-1]))*energy.TWh.to.EJ,rep(NA,len.frida.data-44)))
fossil.energy.calibration          <- data.frame(as.factor(seq(1980,2150)),abind(as.numeric(t(fossil.energy.calibration[,-1]))*energy.TWh.to.EJ,rep(NA,len.frida.data-44)))
total.energy.calibration           <- data.frame(as.factor(seq(1980,2150)),abind(as.numeric(t(total.energy.calibration[,-1]))*energy.TWh.to.EJ,rep(NA,len.frida.data-44)))
